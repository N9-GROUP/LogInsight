...
void process_line(const char *line, char *filter_levels[], int filter_count,
  ...
  if (should_print_log(line, filter_levels, filter_count, start_date,
                       end_date)) {
    if (print_lines) {
      colorize_log(line);
...
void start_log_monitor(const char *file_name, char *filter_levels[],
  ...
  if (!real_time) {
    ...
    while ((bytes_read = read(fd, buffer + current_length,
                              buffer_size - current_length - 1)) > 0) {
  ...
  while (running) {
    ...
    ssize_t bytes_read =
        read(fd, buffer + current_length, buffer_size - current_length - 1);
